...
render_user_header('Book Resource', $user);
?>
<div class="grid grid-2">
  <section class="card">
    ...
    <p style="color:#6b7280;font-size:14px">Overlapping bookings for the same resource are rejected automatically.</p>
  </section>
  <section class="card">
...
async function loadBookings() {
  ...
  const mine = (data.bookings||[]).filter(b => Number(b.user_id) === myId);
  document.getElementById('bookingTable').innerHTML = mine.length
    ? `<table><thead><tr><th>Resource</th><th>Start</th><th>End</th><th>Status</th><th></th></tr></thead><tbody>
      ${mine.map(b => `<tr>
        <td>${escapeHtml(b.asset_tag)} · ${escapeHtml(b.asset_name)}</td>
        <td>${formatDate(b.start_time)}</td><td>${formatDate(b.end_time)}</td>
...
async function loadCalendar() {
  ...
  if (!assetId) return;
  const res = await fetch('../api/index.php?route=bookings&asset_id=' + assetId, { credentials: 'same-origin' });
...
document.getElementById('bookingForm').addEventListener('submit', async (e) => {
  ...
  const start = String(fd.get('start_time')).replace('T', ' ') + ':00';
  const end = String(fd.get('end_time')).replace('T', ' ') + ':00';
  try {
